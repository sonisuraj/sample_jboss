	...
	app.controller('markForReviewController',['$scope','$http','__env','$location','orderByFilter',function($scope,$http,__env,$location,orderBy) {
...
$scope.Review = function (id){
    ...
    $location.url("/logicalDeviceListView/"+name+"/0/markforreview");
	return true;
	...
	$scope.showData=true;
	$scope.Name =id;
	var urlCall =__env.apiUrl+"/devices/getsetreview/"+id;
	$http.get(urlCall)
		.then(function (response) {
